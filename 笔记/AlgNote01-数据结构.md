...
- 双端队列 (Deque, 只用双端队列其实即可模拟 Stack 与Queue)
  - 栈 (Stack，单调栈变体用于优化个别最优化问题的复杂度)
  - 普通队列 (Queue，单调队列变体用于解决滑动窗口最值或优化最优问题的复杂度)
  
- 树与图专题中，深度优先搜索与宽度优先搜索也会用到堆栈、队列作为辅助结构。
...
| AC0239. 程序自动分析<br/>AC0241. 银行英雄传说<br/>AC0240. 食物链 | 使用并查集维护具有传递性的关系，此三题难度逐题递进，AC0241与AC0240 均能使用边带权并查集或扩展域并查集来解 |
|                                                              |                                                              |
