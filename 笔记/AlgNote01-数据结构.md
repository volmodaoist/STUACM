...
- 使用线段树维护区间信息查询问题
  ...
  - 可持久化先线段树 (主席树[^HJT]，可持久化结构的基础)
    ...
    - 主席树维护值域如果很大则需离散化
  - 线段树常见的问题
    - 为何自顶向下要开 4N 空间，自底向上要开 2N 空间[^ZKW]
    - 如何标记永久化 (i.e. 不再下放标记)



> 线段树是连接原始数组与前缀和的梁桥，如果需要支持区间修改，但是只要单索引查询，桥梁两边同时取差分之后，使用线段树维护这个差分数组的前缀和即可。如果需要支持区间查询，先做差分再维护前缀和的前缀和即可，只要额外维护的 `i*A[i]` 信息即可。这个过程能以树状数组，或者非递归线段树实现。
