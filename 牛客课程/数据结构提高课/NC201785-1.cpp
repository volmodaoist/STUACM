...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        prec[i] = prec[i - 1], ones[i] = ones[i - 1], dp[i] = dp[i - 1];
        if (v[i]) {
            dp[i] = (dp[i - 1] + ones[i] * (i - prec[i])) % MOD;
            prec[i] = i;
            ones[i]++;
        }
    }
    
    // 状态转移的时候，为使逻辑相邻的 i-1th 与 ith 能在物理存储上面相邻，
    // 我们每次循环之前都令 dp[i] = dp[i - 1], 因而加总求和的时候，需要判断仅对v[i]等于 1 状态加总
